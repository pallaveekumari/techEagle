  ...
  try {
    ...
    if (!exist) {
      ...
      try {
        ...
        res
          .status(200)
          .json({ msg: "CartData added successfully", status: true });
      } catch (saveError) {
        console.error("Error saving data:", saveError);
        res
          .status(500)
          .json({ msg: "Error saving data", error: saveError, status: false });
      }
    } else {
      res
        .status(400)
        .json({ msg: "Item Already In The CartList", status: false });
    }
  } catch (err) {
    console.error("Something went wrong:", err);
    res
      .status(500)
      .json({ msg: "Something went wrong", error: err, status: false });
  }
};
...
const handleUpdateQuantity = async (req, res) => {
  try {
    ...
    console.log(data);
    let updatedData = await cartModel.updateOne(
      { _id: id, userId: userId },
      { $set: { qty: data.qty + type } }
    );
    res
      .status(200)
      .json({ msg: "Updated Successfully", updatedData, status: true });
  } catch (err) {
    res
      .status(400)
      .json({ msg: "Something went wrong", error: err, status: false });
